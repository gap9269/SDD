...
namespace ISurvived
{
    public class AnubisWarrior : Enemy
    {
        Random standRand;

        Rectangle lineOfSight, locustBounds;
        int lineOfSightLength = 375;
        ...
        public AnubisWarrior(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, Rectangle locustBound =  new Rectangle())
            : base(pos, type, g, ref play, cur)
        {
            ...
            distanceFromFeetToBottomOfRectangle = 30;
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            ...
            {
                if (attackState == AnubisWarrior.AttackState.spear || attackState == AnubisWarrior.AttackState.spear3)
                {
                    return new Rectangle(629 * attackFrame, 2772, 629, 396);
                }
                else if (attackState == AnubisWarrior.AttackState.spear2)
                {
                    return new Rectangle(629 * attackFrame, 2772 + 396, 629, 396);
                }
                else if (attackState == AnubisWarrior.AttackState.summonHands)
                {
                    if (attackFrame < 4)
        ...
        public void AttackOtherObject()
        {
            ...
            if (enemyState != EnemyState.attacking)
            {
                ...
                if (attackType == 0)
                    attackState = AnubisWarrior.AttackState.spear;
                else if (attackType == 1)
                    attackState = AnubisWarrior.AttackState.spear2;
                else
                    attackState = AnubisWarrior.AttackState.spear3;
            ...
            #region SMACK ATTACK
            if (attackState == AnubisWarrior.AttackState.spear || attackState == AnubisWarrior.AttackState.spear2 || attackState == AnubisWarrior.AttackState.spear3)
            {
                ...
                if (attackFrame > 4)
                {
                    ...
                    else
                    {
                        ...
                        attackState = AnubisWarrior.AttackState.none;
                    }
                }
            }
            #endregion
                ...
                if (!summoningLocust)
                {
                    ...
                    switch (priority)
                    {
                        case Priority.melee:
                            ...
                            else if ((attackCooldown <= 0 || handCooldown <= 0) && knockedBack == false && enemyState != EnemyState.attacking)
                            {
                                if (horizontalDistanceToPlayer > meleeRange && horizontalDistanceToPlayer <= maxHandDistance && handCooldown <= 0)
                                    attackState = AnubisWarrior.AttackState.summonHands;
                        ...
                        case Priority.range:
                            ...
                            //IF YOU CAN ATTACK, CHOOSE WHAT TO DO BASED ON DISTANCE FROM PLAYER
                            else if ((attackCooldown <= 0 || handCooldown <= 0) && knockedBack == false && enemyState != EnemyState.attacking && currentPlat != null)
                            {
                                if (horizontalDistanceToPlayer > meleeRange && horizontalDistanceToPlayer <= maxHandDistance && handCooldown <= 0)
                                {
                                    String soundEffectName = "enemy_anubis_summon_0" + Game1.randomNumberGen.Next(1, 3).ToString();
