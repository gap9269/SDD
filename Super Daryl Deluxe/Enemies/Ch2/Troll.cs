...
namespace ISurvived
{
    ...
    public class FieldTroll : Enemy
    {
        ...
        int maxKickCooldown = 300;
        int minKickCooldown = 200;

        int maxSpitCooldown = 1200;
        int minSpitCooldown = 400;

        int maxFallCooldown = 800;
        ...
        public FieldTroll(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, TreasureChest chest = null)
            : base(pos, type, g, ref play, cur)
        {
            ...
            kickCooldown = moveTime.Next(minKickCooldown, maxKickCooldown);
            spitCooldown = moveTime.Next(minSpitCooldown, maxSpitCooldown);

            goblinSpitAmmo = 10;

            notEffectiveRangedMelee = AttackType.RangedOrMelee.none;
        ...
        public void Spit()
        {
            if (moveFrame == 2 && frameDelay == 4)
            {
                ...
                if (dis < 1800)
                {
                    SoundEffectInstance inst = trollSounds["enemy_troll_summon"].CreateInstance();
                    Sound.PlaySoundInstance(inst, "enemy_troll_summon", false, vitalRec.Center.X, vitalRec.Center.Y, 600, 500, 1800);
            ...
            if (frameDelay == 0)
            {
                ...
                if (moveFrame == 3)
                {
                    if (!facingRight)
                    {
                        ...
                        game.CurrentChapter.CurrentMap.AddEnemyToEnemyList(spit);
                    }
                    else
                    {
                ...
                if (moveFrame == 4)
                {
                    for (int i = 0; i < game.CurrentChapter.CurrentMap.EnemiesInMap.Count; i++)
                    {
                        if (game.CurrentChapter.CurrentMap.EnemiesInMap[i] is Goblin && (game.CurrentChapter.CurrentMap.EnemiesInMap[i] as Goblin).Kicked == false && game.CurrentChapter.CurrentMap.EnemiesInMap[i].VitalRec.Intersects(kickRec))
                        {
                            ...
                            float dis = Vector2.Distance(new Vector2(rec.Center.X, rec.Center.Y), new Vector2(player.Rec.Center.X, player.Rec.Center.Y));
                            if ( dis < 1800)
                                game.TempPlayHitSound(rec.Center.X, rec.Center.Y);
                    ...
                    {
                        player.TakeDamage(kickDamage, level);
        ...
        public override void Update(int mapwidth)
        {
            ...
            if (currentFart != null)
            {
                if (currentFart.Update() == true)
                    playerHitByFart = true;
            ...
            if (!respawning)
            {
                if (hostile)
                {
                    ...
                    else if (health < maxHealth * .75f)
                    {
                        ...
                        minSpitCooldown = 350;
                        maxSpitCooldown = 1050;

                        maxAttackCooldown = 125;
                    }
                }
                if (hitPauseTimer <= 0)
                {
            ...
            if (enemyState == EnemyState.moving && (moveFrame == 2 || moveFrame == 9))
            {
                ...
                if (frameDelay == 5)
                {
                    ...
                    SoundEffectInstance inst = trollSounds["enemy_troll_walk_0" + walkSound].CreateInstance();
                    Sound.PlaySoundInstance(inst, "enemy_troll_walk_0" + walkSound, false, vitalRec.Center.X, vitalRec.Center.Y, 600, 1000, 1800);
                    currentlyPlayingSounds.Add(inst);
                }
            }
        ...
        public void Fall()
        {
            if (moveFrame == 0 && frameDelay == 5)
            {
                ...
                SoundEffectInstance inst = trollSounds["enemy_troll_fall"].CreateInstance();
                Sound.PlaySoundInstance(inst, "enemy_troll_fall", false, vitalRec.Center.X, vitalRec.Center.Y, 600, 500, 1800);
                currentlyPlayingSounds.Add(inst);
            }
            ...
            if (frameDelay <= 0)
            {
                ...
                if (moveFrame == 10)
                {
                    SoundEffectInstance inst = trollSounds["enemy_troll_getup"].CreateInstance();
                    Sound.PlaySoundInstance(inst, "enemy_troll_getup", false, vitalRec.Center.X, vitalRec.Center.Y, 600, 500, 1800);
                    currentlyPlayingSounds.Add(inst);
                }
        ...
        public override void Attack(int damage, Vector2 kb)
        {
            ...
            if (enemyState != EnemyState.attacking)
            {
                ...
                //if (moveFrame == 0 && frameDelay == 5)
                SoundEffectInstance inst = trollSounds["enemy_troll_attack"].CreateInstance();
                Sound.PlaySoundInstance(inst, "enemy_troll_attack", false, vitalRec.Center.X, vitalRec.Center.Y, 600, 500, 2000);
                currentlyPlayingSounds.Add(inst);
            }
            ...
            if (frameDelay == 0)
            {
                ...
                if (attackFrame == 19)
                {
                    ...
                    if(health < maxHealth * .5f)
                        ran = moveTime.Next(0, 2);

                    if (ran == 1)
                    {
                        SoundEffectInstance inst = trollSounds["enemy_troll_fart"].CreateInstance();
                        Sound.PlaySoundInstance(inst, "enemy_troll_fart", false, vitalRec.Center.X, vitalRec.Center.Y, 600, 500, 2000);
                        currentlyPlayingSounds.Add(inst);
