...
namespace ISurvived
{
    public class CommanderGoblin : Enemy
    {
        ...
        public CommanderGoblin(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur)
            : base(pos, type, g, ref play, cur)
        {
                ...
                rec = new Rectangle((int)position.X, (int)position.Y, 208, 154);
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            ...
            {
                if (kicked)
                    return new Rectangle(942, 405, 208, 154);
                else
                    return new Rectangle(0, 405, 208, 154);
            }
            else if (landing)
                return new Rectangle((157 * moveFrame) + 157, 405, 208, 154);
            else if (knockedBack || isStunned)
                return new Rectangle(1040, 462, 208, 154);
            else
            {
                switch (enemyState)
                {
                    case EnemyState.standing:
                        if (standState == 0)
                            return new Rectangle(0, 0, 208, 154);
                        else
                            return new Rectangle(208 * moveFrame, 154, 208, 154);
                    case EnemyState.moving:
                        return new Rectangle(208 * moveFrame + 208, 0, 208, 154);
                    case EnemyState.attacking:
                        if (swiping)
                            return new Rectangle(208 * attackFrame + 832, 154, 208, 154);
                        //Spitting
                        return new Rectangle(208 * attackFrame, 462, 208, 154);
        ...
        public override void Attack(int damage, Vector2 kb)
        {
            ...
            if (attackFrame > 1)
            {
                ...
                else if (attackFrame == 3 && spitting && frameDelay == 2)
                {
                    ...
                    if (facingRight)
                    {
                        arrow = new Projectile((int)vitalRec.X + VitalRecWidth + 20, (int)vitalRec.Y + 73, 60, new Vector2(1, 0), (float)Math.PI, spitDamage, new Vector2(5, -5), 1, 0, 10, Projectile.ProjType.goblinSpit, level);
                    }
                    else
                    {
                        arrow = new Projectile((int)vitalRec.X - 20, (int)vitalRec.Y + 73, 60, new Vector2(-1, 0), 0f, spitDamage, new Vector2(5, -5), 1, 0, 10, Projectile.ProjType.goblinSpit, level);
                    }
        ...
        public override void Draw(SpriteBatch s)
        {
            ...
            if (!facingRight)
            {
                ...
                if (attackFrame == 2 && swiping)
                    s.Draw(game.EnemySpriteSheets[name], attackExtensionRec, new Rectangle(1116, 308, 51, 75), Color.White);
            }
            ...
            if (facingRight)
            {
                ...
                if (attackFrame == 2 && swiping)
                    s.Draw(game.EnemySpriteSheets[name], attackExtensionRec, new Rectangle(1116, 308, 51, 75), Color.White, 0f, Vector2.Zero, SpriteEffects.FlipHorizontally, 0f);
            }
            #endregion
