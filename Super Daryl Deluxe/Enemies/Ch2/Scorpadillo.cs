...
namespace ISurvived
{
    public class Scorpadillo : Enemy
    {

        int meleeRange = 150;
        int stabDamage = 25;
        int chanceOfPoison = 20;

        Boolean ballForm = false;
        Boolean unrolling = false;
        Boolean rolling = false;

        public static Dictionary<String, SoundEffect> ratSounds;

        int ballTimer;
        int ballCooldown;

        public Scorpadillo(Vector2 pos, String name, Game1 g, ref Player play, MapClass cur)
            : base(pos, name, g, ref play, cur)
        {
            ...
            level = 14;
            experienceGiven = 12;
            rec = new Rectangle((int)position.X, (int)position.Y, 454, 210);
            currentlyInMoveState = false;
            enemySpeed = 5;
            tolerance = 8;
            maxHealthDrop = 5;
            moneyToDrop = .06f;
            vitalRec = new Rectangle(100, 100, 160, 100);

            distanceFromFeetToBottomOfRectangle = 16;

            rectanglePaddingLeftRight = 150;
        }
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            ...
            {
                if (frame < 9)
                    return new Rectangle(frame * 454, 840, 454, 210);
                else
                    return new Rectangle((frame - 9) * 454, 1050, 454, 210);
            }
            else if (knockedBack || isStunned)
                return new Rectangle(2724, 0, 454, 210);
            if (ballForm)
            {
                return new Rectangle(3178, 0, 454, 210);
            }
            else if (unrolling)
            {
                if (frame < 9)
                    return new Rectangle(frame * 454, 1260, 454, 210);
                else
                    return new Rectangle((frame - 9) * 454, 1470, 454, 210);
            }
            else
            {
                switch (enemyState)
                {
                    case EnemyState.standing:
                        return new Rectangle(frame * 454, 210, 454, 210);

                    case EnemyState.moving:
                        return new Rectangle(frame * 454, 0, 454, 210);

                    case EnemyState.attacking:
                        if(attackFrame < 7)
                            return new Rectangle(attackFrame * 454, 420, 454, 210);
                        else
                            return new Rectangle((attackFrame - 7) * 454, 630, 454, 210);
        ...
        public override void Move(int mapWidth)
        {
            ...
            else if ((hostile == false || distanceFromPlayer > 1700 || verticalDistanceToPlayer > 350 || IsAbovePlayer()) && !knockedBack && enemyState != EnemyState.attacking)
            {
            ...
            else if (hostile && distanceFromPlayer <= 1700 && !knockedBack)
            {
                ...
                else if (attackCooldown <= 0 && knockedBack == false && horizontalDistanceToPlayer <= meleeRange && enemyState != EnemyState.attacking)
                {
                    ...
                    {
                        facingRight = false;
                        attackRec = new Rectangle(vitalRec.X - 150, vitalRec.Y - 20, 200, 100);
                    }
                    else
                    {
                        attackRec = new Rectangle(vitalRec.X + 110, vitalRec.Y - 20, 200, 100);
                        facingRight = true;
                    }
        ...
        public override void TakeHit(int damage, Vector2 kbvel, Rectangle collision, AttackType.AttackTypes skillType, AttackType.RangedOrMelee meleeOrRanged)
        {
            ...
            if (!rolling && ballCooldown <= 0 && !unrolling && !ballForm)
            {
                rolling = true;
            }
        }
        ...
        public override void Draw(SpriteBatch s)
        {
            ...
            if (health < maxHealth)
            {
                ...
                if (facingRight)
                {
                    Game1.OutlineFont(Game1.font, s, "Lv. " + level + " " + displayName, 1, (int)(rec.X + rec.Width / 2 - measX / 2 + 6), healthBoxRec.Y - 25 - 2, Color.Black, Color.White);
                }
                else
                {
