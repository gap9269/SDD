...
namespace ISurvived
{
    public class OutsideStoneFortDemo : MapClass
    {
        ...
        public OutsideStoneFortDemo(List<Texture2D> bg, Game1 g, ref Player play)
            : base(bg, g, ref play)
        {
            yScroll = true;
            zoomLevel = .8f;
        ...
        public override void LoadContent()
        {
            ...
            if (game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"] == false)
            {
        ...
        public override void ResetMapAssetsOnEnter()
        {
            ...
            if (!game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"] && game.ChapterTwoDemo.ChapterTwoBooleans["spawnedOutsideCampGuards"] && enemiesInMap.Count == 0)
            {
                game.ChapterTwoDemo.ChapterTwoBooleans["spawnedOutsideCampGuards"] = false;
            }
        }
        ...
        public override void Update()
        {
            ...
            if (!game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"] && !game.ChapterTwoDemo.ChapterTwoBooleans["spawnedOutsideCampGuards"])
            {
                //Spawn initial enemies
                AnubisWarriorDemo ben = new AnubisWarriorDemo(new Vector2(3750, -205 - 396 * 1.1f), "Anubis Warrior", game, ref player, this, new Rectangle(500, 300, mapRec.Width - 600, 500));
                ben.Hostile = false;
                ben.FacingRight = false;
                ben.SpawnWithPoof = false;
                AddEnemyToEnemyList(ben);

                BomblinDemo ben1 = new BomblinDemo(new Vector2(1500, 16 - 371 * 1.1f), "Bomblin", game, ref player, this);
                ben1.SpawnWithPoof = false;
                AddEnemyToEnemyList(ben1);

                GoblinDemo ben2 = new GoblinDemo(new Vector2(2700, 266 - 154), "Goblin", game, ref player, this);
                ben2.Hostile = false;
                ben2.SpawnWithPoof = false;
                AddEnemyToEnemyList(ben2);

                game.ChapterTwoDemo.ChapterTwoBooleans["spawnedOutsideCampGuards"] = true;
            }

            //Change the map once the bridge has fallen
            if (game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"])
            {
            ...
            if (CheckSwitch(doorSwitch) && !game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"])
            {
                if (!game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"])
                   game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"] = true;
        ...
        public override void SetDestinationPortals()
        {
            ...
            portals.Add(toCampMiddle, StoneFortCentralDemo.ToOutsideCamp);
        }
...
null, null, null, null, Game1.camera.Transform);
            ...
            if (!game.ChapterTwoDemo.ChapterTwoBooleans["campDoorFallen"] && game.CurrentChapter.state != Chapter.GameState.Cutscene)
            {
