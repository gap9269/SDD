...
namespace ISurvived
{
    public class BomblinDemo : Enemy
    {
        ...
        public BomblinDemo(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, EnemyLevel enemyLevel = EnemyLevel.ten)
                ...
                if (attackFrame > 1)
                {
                    if ((player.CheckIfHit(attackRec) || player.CheckIfHit(attackRec)) && player.InvincibleTime <= 0)
                    {
                        ...
                        if (randomExplode == 0)
                        {
                            ...
                            TakeHit(bombDamage * 2, kb1, new Rectangle(attackRec.X, attackRec.Y, 190, 190), AttackType.AttackTypes.Blunt, AttackType.RangedOrMelee.Melee);
        ...
        public override void ImplementGravity()
        {
            ...
            if (currentPlat != null)
            {
                if (position.X < currentPlat.Rec.X - 100)
                {
                    velocity.X = 0;
                    position.X = currentPlat.Rec.X - 100;
                }
                if (position.X + rec.Width > currentPlat.Rec.X + currentPlat.Rec.Width + 100)
                {
                    velocity.X = 0;
                    position.X = (currentPlat.Rec.X + currentPlat.Rec.Width) - rec.Width + 100;
                }
            }
            #endregion
