...
namespace ISurvived
{
    public class FlufflesTheRat : Enemy
    {
        ...
        public FlufflesTheRat(Vector2 pos, String name, Game1 g, ref Player play, MapClass cur)
            ...
            if (frameDelay <= 0)
            {
                ...
                if (moveFrame > 5)
                {
                    ...
                    if (attackAfter)
                    {
                        ...
                        String soundEffectName = "enemy_rat_attack_0" + randomSound;
                        Sound.PlaySoundInstance(FlufflesTheRat.ratSounds[soundEffectName], soundEffectName, false, rec.Center.X, rec.Center.Y, 600, 500, 2000);
        ...
        public override void Move(int mapWidth)
        {
            ...
            else if (hostile && distanceFromPlayer <= 1700 && !knockedBack)
            {
                ...
                else if (horizontalDistanceToPlayer > meleeRange && enemyState != EnemyState.attacking && !pullingKnife && !knockedBack)
                    MoveTowardPlayer(mapWidth);
