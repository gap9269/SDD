...
namespace ISurvived
{
    public class NurseGoblin : Enemy
    {
        ...
        int damageThisHeal = 0;
        int damageToCancelHealing = 100;
        int altAttackCooldown = 200;
        int maxHealingCooldown = 400;
        ...
        public NurseGoblin(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, EnemyLevel enemyLevel = EnemyLevel.ten)
            : base(pos, type, g, ref play, cur)
        {
                ...
                runLeft = false;
                runRight = false;
                healRec = new Rectangle(0, 0, 447, 269);
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            ...
            {
                if (moveFrame < 10)
                    return new Rectangle(moveFrame * 385, 696, 385, 232);
                else
                    return new Rectangle((moveFrame - 10) * 385, 928, 385, 232);
            }
        ...
        public void Heal()
        {
            ...
            if (frameDelay == 0)
            {
                ...
                if (moveFrame == 1)
                    Sound.PlaySoundInstance(Goblin.goblinSounds["enemy_goblin_nurse_heal"], "enemy_goblin_nurse_heal", false, rec.Center.X, rec.Center.Y, 600, 500, 2000);
            ...
            if (moveFrame > 11)
            {
                ...
                if (healLoop == 5)
                {
                    ...
                    healing = false;
                    enemyState = EnemyState.standing;
                    healingCooldown = maxHealingCooldown;
                    canBeKnockbacked = true;
                }
            }

            if (moveFrame > 0 && moveFrame < 9 && frameDelay == 5)
            {
                foreach (Enemy enemy in currentMap.EnemiesInMap)
                {
                    if (enemy.Health < enemy.MaxHealth)
                    {
                        if (healRec.Intersects(enemy.VitalRec))
                        {
                            enemy.Health += (enemy.MaxHealth / 80);
        ...
        public void FindEnemyToHeal()
        {
            foreach (Enemy enemy in currentMap.EnemiesInMap)
            {
                if ((!(enemy is NurseGoblin) || (enemy as NurseGoblin).healing == false) && enemy.Health < enemy.MaxHealth)
                {
                    if (healRec.Intersects(enemy.VitalRec))
                    {
                        healing = true;
        ...
        public override void Move(int mapWidth)
        {
            ...
            if (healingCooldown <= 0 && enemyState != EnemyState.attacking && !flying && !landing && !healing && !knockedBack)
            {
                FindEnemyToHeal();
            }
        ...
        public override void Draw(SpriteBatch s)
        {
            ...
            if (!facingRight)
            {
                ...
                {
                    if (moveFrame < 9)
                        s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(moveFrame * 385, 1160, 385, 232), Color.White * alpha);

                    s.Draw(game.EnemySpriteSheets[name], healRec, GetSourceRectangle(moveFrame), Color.White * alpha);
                    if (moveFrame > 3 && moveFrame < 10)
                    {
                        if (moveFrame != 10)
                            s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(moveFrame * 385, 1392, 385, 232), Color.White * alpha);
                        else
                            s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(0, 1624, 385, 232), Color.White * alpha);
                    }
                }
            }
            ...
            if (facingRight)
            {
                ...
                {
                    if(moveFrame < 9)
                        s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(moveFrame * 385, 1160, 385, 232), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.FlipHorizontally, 0f);

                    s.Draw(game.EnemySpriteSheets[name], healRec, GetSourceRectangle(moveFrame), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.FlipHorizontally, 0f);

                    if (moveFrame > 3 && moveFrame < 10)
                    {
                        if (moveFrame != 10)
                            s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(moveFrame * 385, 1392, 385, 232), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.FlipHorizontally, 0f);
                        else
                            s.Draw(game.EnemySpriteSheets[name], healRec, new Rectangle(0, 1624, 385, 232), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.FlipHorizontally, 0f);
                    }
                }
            }
            #endregion
        ...
        public override void TakeHit(int damage, Vector2 kbvel, Rectangle collision, AttackType.AttackTypes skillType, AttackType.RangedOrMelee meleeOrRanged)
        {
            ...
            if (healing)
            {
                damageThisHeal += (int)(damage * (250f / (250f + tolerance)));

                if (damageToCancelHealing <= damageThisHeal)
                {
                    damageThisHeal = 0;
                    healing = false;
                    healingCooldown = maxHealingCooldown;
        ...
        public override void DropItem()
        {
            ...
            if (dropType < 25)
            {
                if (EnemyType == "Nurse Goblin")
                    currentMap.Drops.Add(new EnemyDrop("First Aid Kit", new Rectangle(rec.Center.X, rec.Center.Y, dropDiameter, dropDiameter)));
                else
            ...
            else if (dropType < 43)
            {
                ...
                {
                    currentMap.Drops.Add(new EnemyDrop(new AutomaticFirstAid(), new Rectangle(rec.Center.X, rec.Center.Y, dropDiameter, dropDiameter)));
