...
namespace ISurvived
{
    class ForkEnemy : SteeringEnemy
    {
        public ForkEnemy(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, Rectangle bound)
            : base(pos, "Garden Beast", g, ref play, cur, bound)
        {
            ...
            level = 3;
            experienceGiven = 1;
            rec = new Rectangle((int)position.X, (int)position.Y, 158, 270);
            tolerance = 2;
            vitalRec = new Rectangle(rec.X + 20, rec.Y + 10, 75, 200);
            maxHealthDrop = 5;
            moneyToDrop = .03f;
        }
        ...
        {
            return new Rectangle(0, 0, 158, 270);
        }
