...
namespace ISurvived
{
    public class MaracasHermanos : Enemy
    {
        ...
        public MaracasHermanos(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur)
            : base(pos, type, g, ref play, cur)
        {
            ...
            level = 7;
            experienceGiven = 21;
            rec = new Rectangle((int)position.X, (int)position.Y, 467, 203);
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            if (knockedBack || isStunned)
                return new Rectangle(2335, 0, 467, 203);
            else
            {
                switch (enemyState)
                {
                    case EnemyState.standing:
                        if (moveFrame < 6)
                            return new Rectangle(934 + (467 * moveFrame), 406, 467, 203);
                        else
                            return new Rectangle(467 * (moveFrame - 6), 609, 467, 203);
                    case EnemyState.moving:
                        return new Rectangle(467 * moveFrame, 0, 467, 203);
                    case EnemyState.attacking:
                        if (attackFrame < 8)
                            return new Rectangle(467 * attackFrame, 203, 467, 203);
                        else
                            return new Rectangle(467 * (attackFrame - 8), 206, 467, 203);
                }
            }
        ...
        public override void Move(int mapWidth)
        {
            ...
            if (isStunned == false)
            {
                ...
                else if ((horizontalDistanceToPlayer > attackRange || attackCooldown > 0) && knockedBack == false && enemyState != EnemyState.attacking)
                {
                ...
                else if (enemyState != EnemyState.attacking)
                {
                    ...
                    if (attackCooldown <= 0)
                    {
                        ...
                        if (facingRight)
                        {
                            attackRec = new Rectangle(vitalRec.X + vitalRec.Width / 2, vitalRec.Y + 30, 250, 150);
                        }
                        else
                        {
                            attackRec = new Rectangle(vitalRec.X + vitalRec.Width / 2 - 250, vitalRec.Y + 30, 250, 150);
                        }
                    }
                }
        ...
        public override void Attack(int damage, Vector2 kb)
        {
            ...
            if (attackFrame > 5 && attackFrame < 8)
            {
        ...
        public override void DropItem()
        {
            ...
            if (dropType < 25)
            {
                currentMap.Drops.Add(new EnemyDrop("Mexican Jumping Beans", new Rectangle(rec.Center.X, rec.Center.Y, dropDiameter, dropDiameter)));
            }
            else if (dropType < 35)
            {
