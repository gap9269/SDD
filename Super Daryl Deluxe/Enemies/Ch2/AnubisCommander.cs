...
namespace ISurvived
{
    public class AnubisCommander : Enemy
    {
        Random standRand;
        ...
        public AnubisCommander(Vector2 pos, String type, Game1 g, ref Player play, MapClass cur, Rectangle locustBound)
        ...
        public override Rectangle GetSourceRectangle(int frame)
        {
            ...
            {
                if (attackState == AnubisCommander.AttackState.spear || attackState == AnubisCommander.AttackState.spear3)
                {
                    return new Rectangle(775 * attackFrame, 0, 775, 458);
                }
                else if (attackState == AnubisCommander.AttackState.spear2)
                {
                    return new Rectangle(775 * attackFrame, 458, 775, 458);
                }
                else if (attackState == AnubisCommander.AttackState.summonHands)
                {
                    if (attackFrame < 2)
            ...
            else if (hostile && distanceFromPlayer <= hostileDistance)
            {
                ...
                if (currentMap.EnemiesInMap.Count < 20)
                {
                    if (locustCooldown <= 0 && enemyState != EnemyState.attacking)
                    {
                        ...
                        if (PositionX < 200)
                            facingRight = false;
                        if (PositionX + rec.Width > currentMap.MapWidth + 250)
                            facingRight = true;
                ...
                if (!summoningLocust)
                {
                    ...
                    else if ((attackCooldown <= 0 || handCooldown <= 0) && knockedBack == false && enemyState != EnemyState.attacking)
                    {
                        if (horizontalDistanceToPlayer > meleeRange && horizontalDistanceToPlayer <= maxHandDistance && handCooldown <= 0)
                            attackState = AnubisCommander.AttackState.summonHands;
                        else if (horizontalDistanceToPlayer <= meleeRange)
                        {
                            ...
                            else if (attackCooldown <= 0)
                            {
                                ...
                                if (attackType == 0)
                                    attackState = AnubisCommander.AttackState.spear;
                                else if (attackType == 1)
                                    attackState = AnubisCommander.AttackState.spear2;
                                else
                                    attackState = AnubisCommander.AttackState.spear3;
                            }
                            else
                                Stand();
                        }
                    ...
                    if (attackState == AnubisCommander.AttackState.spear || attackState == AnubisCommander.AttackState.spear2 || attackState == AnubisCommander.AttackState.spear3)
                    {
                        Vector2 kb;
            ...
            if (attackState == AnubisCommander.AttackState.spear || attackState == AnubisCommander.AttackState.spear2 || attackState == AnubisCommander.AttackState.spear3)
            {
                ...
                if (attackFrame > 4)
                {
                    ...
                    else
                    {
                        ...
                        attackState = AnubisCommander.AttackState.none;
                    }
                }
            }
            #endregion
